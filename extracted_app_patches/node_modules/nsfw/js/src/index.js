...
let NSFW;
try {
  NSFW = require('../../build/Release/nsfw.node');
} catch (e) {
  NSFW = function JSNSFW(watchPath, eventCallback, options) {
    this.watchPath = watchPath;
    this.eventCallback = eventCallback;
    this.excludedPaths = (options && options.excludedPaths) || [];
    this.watcher = null;

    this.start = async () => {
      try {
        const fsSync = require('fs');
        if (fsSync.watch && typeof this.watchPath === 'string') {
          this.watcher = fsSync.watch(this.watchPath, { recursive: true }, (eventType, filename) => {
            if (typeof this.eventCallback === 'function' && filename) {
              try {
                this.eventCallback([{
                  action: eventType === 'rename' ? 0 : 2,
                  directory: this.watchPath,
                  file: filename
                }]);
              } catch (err) {}
            }
    ...
    this.getExcludedPaths = () => this.excludedPaths;
    this.updateExcludedPaths = async (paths) => {
